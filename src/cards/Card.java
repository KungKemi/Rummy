...
public class Card implements Encodable {
    ...
    public Card(int cardNum, CardType cardType) throws IllegalCardException {
        if (cardNum < 1 || cardNum > 14) {
            ...
            if (cardType != CardType.BLACK_JOKER && cardType != CardType.RED_JOKER) {
                throw new IllegalCardException("cards.Card id is 14 but card type is: " + cardType);
            }
        } else {
            if (cardType == CardType.BLACK_JOKER || cardType == CardType.RED_JOKER) {
                throw new IllegalCardException("cards.Card type is JOKER but card id is: " + cardNum);
            }
        }
     ...
     * cards.Card:cardNum:cardType
     *
     * @return the encoded card instance
     */
    public String encode() {
        return String.format("cards.Card:%d:%s",
                this.cardNum,
                this.cardType);
    }
    ...
    public static Card fromString(String string) throws BadEncodingException {
        ...
        if (!inputArgs[0].equals("cards.Card")) {
            throw new BadEncodingException("Encoded string should start with 'cards.Card'");
        }
        try {
    ...
    public static String encodeString(String string) {
        ...
        return String.format("cards.Card:%d:%s", cardNum, cardType);
    }
